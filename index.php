...
function saveConfig($a){
    ...
    if (!$a) {
        ...
        $stmt = $conn->prepare("INSERT INTO papersize (sizeName, paperWidth, paperHeight, totalX, totalY, active) VALUES(?,?,?,?,?,?)");
        $stmt->bind_param("ssssss", $sizeName, $paperWidth, $paperHeight, $totalX, $totalY, $active);
        $stmt->execute();
    } else {
        if ($active == 1){
            $sql = "UPDATE papersize SET active='0'";
        ...
        $stmt = $conn->prepare("UPDATE papersize SET sizeName = ?, paperWidth = ?, paperHeight = ?, totalX = ?, totalY = ?, active = ? WHERE id = ?");
        $stmt->bind_param("sssssss", $sizeName, $paperWidth, $paperHeight, $totalX, $totalY, $active, $a);    
        $stmt->execute();
    }
    ...
    while ($item = $result->fetch_assoc()) {
        ...
        $table .= '
                <tr>
                    ...
                    <td>'.$item['sizeName'].'</td>
                    <td>'.$item['paperWidth'].'pt</td>
                    <td>'.$item['paperHeight'].'pt</td>
                    <td>'.$item['totalX'].'</td>
                    <td>'.$item['totalY'].'</td>
            ...
            <fieldset>
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                            ...
                            <small id="paperWidth" class="form-text text-muted">Size in points eg.595pt(A4) <a href="https://github.com/Bubblbu/paper-sizes" target="_blank">Guide</a></small>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <small id="paperHeight" class="form-text text-muted">Size in points eg.842pt(A4) <a href="https://github.com/Bubblbu/paper-sizes" target="_blank">Guide</a></small>
                        </div>
                        <div class="form-group">
